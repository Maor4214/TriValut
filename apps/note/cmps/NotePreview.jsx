...
export function NotePreview({ note, onNoteClick, onRemoveNote, onTogglePin }) {
  ...
  function onHandleButtonClick(ev, id) {
    ev.stopPropagation()
  }
  ...
  return (
    ...
    >
      ...
      {type !== 'noteTodos' && <div className="note-txt">{info.txt}</div>}
      {type === 'noteTodos' && <TodoNote info={info} />}
      <div className="note-navbar">
        <button onClick={(ev) => onHandleButtonClick(ev, id)}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="24px"
        ...
        <button onClick={(ev) => handleToggleOptions(ev)}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="24px"
            viewBox="0 -960 960 960"
